...
| **fra-a11y** | 20.04.2026   | [Link to Commit 1] | This commit implements the horizontal layout of the event board: it transforms the display from a single column to a 3-column grid with post cards aligned horizontally. | The view board is a important feature of the app. The code now fetches posts from the /events/{eventId}/posts endpoint on component mount, handles the form submission to create new posts with proper error handling, and dynamically displays all posts in the horizontal grid layout.|
