...
export class ApiService {
  ...
  constructor() {
    this.baseURL = getApiDomain().replace(/\/$/, "");
    this.defaultHeaders = {
   ...
   */
  public async get<T>(endpoint: string, extraHeaders?: Record<string, string>): Promise<T> {
   ...
   * @returns JSON data of type T.
   */
  public async post<T>(endpoint: string, data: unknown, extraHeaders?: Record<string, string>): Promise<T> {
    const url = `${this.baseURL}${endpoint}`;
    const res = await fetch(url, {
      method: "POST",
      headers: { ...this.defaultHeaders, ...extraHeaders },
